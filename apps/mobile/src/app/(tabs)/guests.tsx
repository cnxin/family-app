...
import { Card, EmptyState, IOSSwitch, PrimaryButton, SummaryBand } from '../../components/ui';
...
export default function GuestsScreen() {
  ...
  const anonymizeGuest = useAnonymizeGuest();
  ...
  const confirmedGuests = scheduledVisits.reduce(
    (total, visit) => total + visit.guests.filter((guest) => guest.isAttending === true).length,
    0,
  );
  const waitingGuests = scheduledVisits.reduce(
    (total, visit) => total + visit.guests.filter((guest) => guest.isAttending == null).length,
    0,
  );
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: c.bg }]} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <PageContainer style={[styles.content, desktop && styles.contentDesktop]}>
          ...
          <SummaryBand
            items={[
              { color: c.tint, icon: CalendarDays, label: '待来访', value: scheduledVisits.length },
              { color: c.green, icon: Check, label: '已确认', value: confirmedGuests },
              { color: c.orange, icon: Send, label: '待回复', value: waitingGuests },
            ]}
            style={styles.summaryBand}
            testID="guest-visit-summary"
          />
          ...
          {guestsLoading ? <ActivityIndicator color={c.tint} style={styles.loader} /> : guests?.length ? <View style={styles.guestGrid}>{guests.map((guest) => <Card key={guest.id} style={[styles.guestCard, !guest.isActive && { opacity: 0.62 }]}><View style={styles.guestCardTop}><Text style={{ fontSize: 30 }}>{guest.avatarEmoji}</Text>{guest.anonymizedAt ? <Text style={[t.caption, { color: c.tertiaryLabel }]}>已匿名化</Text> : <IOSSwitch value={guest.isActive} onValueChange={(isActive) => void updateGuest.mutateAsync({ id: guest.id, isActive })} accessibilityLabel={`${guest.name}${guest.isActive ? '启用' : '停用'}`} />}</View><Pressable accessibilityRole="button" disabled={Boolean(guest.anonymizedAt)} onPress={() => setGuestForm(guest)} style={styles.guestMain}><Text style={[t.headline, { color: c.label }]}>{guest.name}</Text><Text numberOfLines={2} style={[t.caption, { color: c.secondaryLabel, marginTop: 5, minHeight: 32 }]}>{guest.anonymizedAt ? '已移除身份资料，历史记录仅保留匿名事实。' : guest.note ?? (guest.isActive ? '暂无家庭备注' : '已停用，不可加入新来访')}</Text></Pressable>{!guest.anonymizedAt ? <Pressable accessibilityRole="button" onPress={() => Alert.alert('匿名化访客', '将移除姓名、头像和备注，并撤销仍有效的邀请链接。历史来访与选择会保留为匿名记录。', [{ text: '返回' }, { text: '匿名化', style: 'destructive', onPress: () => void anonymizeGuest.mutateAsync(guest.id) }])} style={[styles.anonymizeAction, { backgroundColor: c.fill }]}><Text style={[t.caption, { color: c.secondaryLabel }]}>匿名化</Text></Pressable> : null}</Card>)}</View> : <Card><EmptyState icon={UserPlus} iconBackground={c.tintSoft} iconColor={c.tint} title="还没有访客档案" hint="添加常来访客，不会为其创建正式账号。" /></Card>}
        </PageContainer>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  addButton: { minHeight: 44, paddingHorizontal: 14, borderRadius: radius.sm, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  summaryBand: { marginTop: 20 },
  ...
  visitCard: { padding: 14 }, visitHeader: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, wifiAssignment: { minHeight: 44, borderRadius: radius.sm, paddingHorizontal: 10, marginTop: 11, flexDirection: 'row', alignItems: 'center', gap: 7 }, participantList: { marginTop: 13, paddingTop: 8, borderTopWidth: StyleSheet.hairlineWidth, gap: 4 }, participantRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 8 },
  mealRequests: { marginTop: 12, paddingTop: 10, borderTopWidth: StyleSheet.hairlineWidth, gap: 7 }, mealRequestHeader: { flexDirection: 'row', alignItems: 'center', gap: 6 }, mealRequestRow: { minHeight: 60, borderRadius: radius.sm, padding: 9, flexDirection: 'row', alignItems: 'center', gap: 8 }, mealRequestActions: { flexDirection: 'row', gap: 6 }, mealRequestAction: { width: 44, height: 44, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' },
  inviteButton: { minHeight: 44, paddingHorizontal: 10, borderRadius: radius.sm, flexDirection: 'row', alignItems: 'center', gap: 4 }, iconAction: { width: 44, height: 44, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' }, visitActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 8, marginTop: 10 }, textAction: { minHeight: 44, paddingHorizontal: 12, borderRadius: radius.sm, justifyContent: 'center' },
  addGuest: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 4 }, guestGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, guestCard: { width: '47%', minWidth: 150, flexGrow: 1, padding: 13 }, guestCardTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 },
  guestMain: { minHeight: 56, justifyContent: 'center' },
  anonymizeAction: { minHeight: 44, alignSelf: 'flex-start', borderRadius: radius.sm, paddingHorizontal: 10, justifyContent: 'center', marginTop: 6 },
  wifiCard: { padding: 14 }, wifiCardTop: { flexDirection: 'row', alignItems: 'center', gap: 10 }, wifiProfileMain: { flex: 1, minWidth: 0, minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 10 }, wifiMark: { width: 36, height: 36, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' }, wifiChoices: { marginTop: 14, borderTopWidth: StyleSheet.hairlineWidth, gap: 2, paddingTop: 6 }, wifiChoice: { minHeight: 52, paddingVertical: 7, flexDirection: 'row', alignItems: 'center', gap: 9 },
  permissionRow: { minHeight: 64, borderRadius: radius.sm, padding: 10, marginTop: 18, flexDirection: 'row', alignItems: 'center', gap: 9 }, permissionMark: { width: 34, height: 34, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' },
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.38)', justifyContent: 'center', padding: 18 }, sheet: { width: '100%', maxWidth: 560, maxHeight: '88%', alignSelf: 'center', borderWidth: 1, borderRadius: radius.md, padding: 18 }, sheetHeader: { minHeight: 44, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }, closeButton: { width: 44, height: 44, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' },
  fieldRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 16 }, input: { minHeight: 44, borderRadius: radius.sm, paddingHorizontal: 11 }, noteInput: { minHeight: 76, borderRadius: radius.sm, paddingHorizontal: 11, paddingTop: 10, textAlignVertical: 'top' }, dateRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 14, alignItems: 'flex-end' }, guestPicker: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, guestChoice: { minHeight: 44, borderWidth: 1, borderRadius: radius.sm, paddingHorizontal: 10, flexDirection: 'row', alignItems: 'center', gap: 5 },

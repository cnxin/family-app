...
compose exec -T db sh -c 'pg_isready -U "$POSTGRES_USER" -d postgres' >/dev/null
...
if [ "$TARGET_DATABASE" = "$ACTIVE_DB" ]; then
  printf 'Refusing to overwrite active production database "%s".\n' "$ACTIVE_DB" >&2
  exit 2
fi

EXISTS=$(compose exec -T db psql -U "$DB_USER" -d postgres -Atc \
...
compose run --rm --no-deps -T -e "DB_NAME=$TARGET_DATABASE" backup-worker \
  node /app/apps/api/node_modules/typeorm/cli.js migration:run \
  -d /app/apps/api/dist/database/data-source.js
...
printf 'Production restore drill complete.\n'
printf 'Database: %s\n' "$TARGET_DATABASE"
printf 'Uploads: %s/uploads\n' "$UPLOAD_PREVIEW_DIR"
printf 'The active production database and uploads were not changed.\n'

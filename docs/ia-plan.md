...
- **缓存与预取**：状态行仅订阅成功的 QueryCache，不挂载业务查询，缺数据不显示、不伪造零；已有列表状态用「已载入」说明数量口径，未定义摘要的域只显示名称。`/home` 导航意图预取资产、来访（已含点菜请求）、进行中出行、在库批次、投票，管理员额外预取兑换、当月财务概览、备份看板；查询键和端点复用现有 hook。F5 再接消费者、按来访日期查菜单和今天页预取，本轮不实现留意规则 / 卡片。
- **兼容修正**：按 §3 恢复 `/eat` → `/eat/order`、`/schedule` → `/schedule/calendar`、`/life` → `/life/media`；`/house` / `/me` → `/home`，五个根路径均保留查询参数；现有叶子与旧路径重定向不变。
- **验收清单**：新增 `/home` 冒烟；双视口覆盖资产直达、搜索、成功缓存与异步状态更新、冷启动无请求、按角色过滤预取 / 图块 / 页底入口、五个根落点和旧深链。四张图输出 `.tmp-shots/f2-{390x844,1280x800}-{light,dark}.png`；最终结果回填进度表。无 packages / API 改动。教训 18 与功能代码同提交。
...
| F2 家里页 | ◐ | 待提交 · `feat(web): 家里页，低频功能的启动台` | 启动台、缓存状态、角色过滤预取及旧根落点修正已实现；typecheck / lint 通过（API 43 条既有 warning）；全量新端浏览器 157 passed / 5 skipped / 0 failed，四图已逐张复看；提交后等待 CI 并回填；详见 F2 备注。 |
| F3 模块状态端点 | ☐ | | |
| F4 隐身 / 开启 / 置顶 | ☐ | | |
| F5 需要留意 | ☐ | | |
| F6 ⌘K 动作 | ☐ | | |

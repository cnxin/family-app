...
| F5 需要留意 | ◐ | `fd33353` · `feat(api): 今天页留意端点，规则在服务端按家庭日期计算` | F5a 已完成：端点置于独立 `TodayModule`（今天页聚合职责清晰，避免把跨域规则塞回任一业务域）；规则 provider 与集中阈值 `ATTENTION_THRESHOLDS` 位于 `apps/api/src/today/today-attention.rules.ts`，按家庭并行精确查询；权限过滤使用 `manage_guests` / `manage_points` / `manage_finance` / `manage_integrations`，投票按本人未投过滤。黑盒包含 60 件资产且断言 `count = 60`；业务规则无 `new Date()` 取当前时间残留，统一使用注入 `Clock` + 家庭时区。全量 `corepack pnpm test:api` 通过；已 push，CI `35679975708` 五项全绿。F5b 进行中。 |
| F6 ⌘K 动作 | ☐ | | |
| F7 家庭设置 | ☐ | | |
| F8 收口 | ☐ | | |

...
这是整套方案里最要紧的一块：低频域靠它「来找你」。**2026-09-21 变更：规则移到服务端。** 原计划在浏览器从各域列表拼规则，改为由服务端按家庭日期计算，原因是：① 列表接口有条数上限，前端从截断列表计数会错；② 首页需要发 8～10 个请求，弱网和中继下不可接受；③ 结果要被 Web、App、小程序、小管家和推送摘要复用。

#### F5a · 服务端

新增 `GET /today/attention`。先核对今天页现有聚合端点：若已有聚合模块就在其中扩展，否则新建小模块；具体位置在实现汇报中说明。

- **响应契约**（放 `packages/contracts`）：`{ today: PlainDate, items: AttentionItem[] }`。
  `AttentionItem = { key, domain, kind, count, dueOn?: PlainDate, overdue: boolean, entity?: { id, name } }`。`count === 1` 时带 `entity`，`count > 1` 时不带；`key` 必须稳定（同一件事今天和明天相同），供「稍后」使用。更新 `docs/api-inventory.md`：277 → 278。
- **规则与查询**：规则和阈值沿下表；阈值常量集中放一处，每条规则一个独立 provider。使用精确 SQL（按日期范围 `WHERE`，不读列表接口、不受条数上限影响），每条查询都带 `household_id`，一次请求内并行。智能家居只保留注释占位，E 阶段接入。
- **日期与时钟**：「今天」来自注入的 Clock + 家庭时区，日期运算统一使用 `packages/shared/src/date.ts`，不得用 `new Date()` 取得当前时间。
- **权限与模块**：服务端按现有 capability 过滤；普通成员不得收到预算、兑换审批、备份、访客点菜请求；投票只返回当前成员尚未参与的投票。读取 `household_module_overrides`：`override = off` 的域不出条目，空域自动隐身不抑制留意。具体 capability 在实现汇报中列明。
- **合并与排序**：一个域最多一条（多件合并为 `count`）；逾期优先，其次 `dueOn` 升序，再按固定域顺序。
- **黑盒**：新增 `apps/api/scripts/today-attention.mjs`，冻结时钟覆盖每条规则的「到期前一天不出 / 阈值当天出 / 逾期出且 `overdue=true` / 办完后消失」，另测家庭零点前后一分钟、普通成员过滤管理员类目、`override=off` 抑制、跨家庭隔离（补进 `household-isolation.mjs`）。必须造 60 件 7 天内到期资产，确认 `count === 60`，证明不受列表上限影响。
...
- **查询与失效**：新增 `useAttention()`，单个查询，`staleTime = 1 分钟`，窗口重新聚焦时重取。所有会改变 F5a 结果的 mutation 成功后都要失效它；可与 `invalidateModules()` 合并为统一入口，也可分开，但实现汇报须逐域列核对表。
- **文案**：集中在一个与 React 无关的纯函数文件，输入 `AttentionItem`，输出 `{ 域标签, 标题, 时间说明, 主动作文案, 跳转路径 }`。`count === 1` 用具体名称（如「净水器滤芯 3 天后该换了」），大于 1 用合并说法（如「3 件资产快到期」）。路径一律经 `routes.ts`。优先考虑放 `packages/shared` 供将来复用；若依赖方向不允许，放 `apps/web/src/lib`，实现汇报注明。配 Vitest 单测。
- **卡片**：域标签 + 时间说明 + 标题 + 主动作按钮 +「稍后」；沿用现有 `Card` / `Button` 和 token，不照搬示意稿样式；暗色通过；触控区 ≥ 44px。
- **稍后**：`localStorage` 记 `{ key, until }`，`until` 为响应 `today` 对应家庭时区的下一日零点；读写 `try/catch`。到点自动回来。
- **四条硬规矩**：一域一卡；必须有一步到位动作；不落库、不做已读未读；最多显示 5 张，其余折叠成「还有 N 件」；一张都没有时整个区块不渲染，不放「一切安好」空态。
- **今天页布局**：保留现有聚合区块和「全家任务」口径，不按示意稿删减；移除「动态」和「未读统计」。顺序：三餐 → 任务（我的在前、全家在后）→ 当日日历与提醒 → 购物 → 需要留意。桌面 ≥1024px 时「需要留意」放右栏；无卡片时左栏撑满、不留空白；其余区块内部不动。
- **家里页与预取**：图块状态行取该域留意条目的简短说法，无条目不显示，不为状态行发额外请求；`/` 和 `/home` 预取 `/today/attention`。
...
- 来访记录没有餐次字段。「去点菜」默认晚餐。点菜页消费 `date` 和 `meal`（仍兼容 F5 的 `mealType`），读完从 URL 抹掉。见下方「深链约定」。
...
读到参数就进入对应状态，处理完从 URL 抹掉，不改页面其他内部。F6 的 `?create=1` 也记在这里。
...
- **统一深链约定（2026-09-20 已拍板）**：`?create=1`（需要区分类型的加 `&kind=expense`）表示「到达该页并进入现成的新建状态」：已有新建弹窗就打开；任务、购物等页内输入则滚动到输入框并聚焦，不为此新增弹窗。处理完把参数从 URL 上抹掉，不改现成表单 / 弹窗内部。只修 F6 动作注册表用到的页面；F0 发现的其他“链接带参但页面未消费”情况仅留盘点备注，不在本轮修。日期和餐次见上方「深链约定」。
...
F5b 盘点之后的决定。半成品里没写明的分支以这里为准。

1. **消息未读角标**：桌面侧栏「消息」旁边显示数字；手机「日程」tab 显示红点，气泡里「消息」旁边显示数字。复用现有未读查询，不加接口。
2. **今天页任务分组**：只分「我的 / 全家」两组。已指派给当前成员的归「我的」，待认领归「全家」。
3. **点菜深链**：点菜页可以消费 `?date=`。读到就把购物车日期切到那一天，再把 `date` 从 URL 抹掉。不改点菜页的其他内部。来访记录没有餐次字段，主动作默认晚餐；链接上若有合法 `mealType` 就用它。
4. **访客还没点菜**：判断改为「当天没有任何菜品」。空菜单行不算已经有菜单。
5. **稍后按成员存储**：键名 `fa.snooze.<memberId>`，和置顶的 `fa.pins.<memberId>` 一样按成员分开。
6. **一域一卡保留**：合并后的条目带 `kinds: string[]`。一张卡里混了多种 `kind` 时，主动作落到该域的列表页，不再只跟排序胜出的那一种。
7. **访客点菜请求的刷新**：访客在公开邀请页提交之后，家里人的今天页靠 1 分钟过期和窗口重新聚焦兜底。本轮不做推送。教训记在 `docs/refactor-plan.md`，指向将来的 `/events` 事件通道。
...
| F5 需要留意 | ☑ | `fd33353` / `b63fee0` / `7273ab0` / `f57fa67` | F5a：`fd33353` 端点在独立 `TodayModule`，规则与 `ATTENTION_THRESHOLDS` 在 `today-attention.rules.ts`，按家庭日期计算；CI `35679975708` 五项全绿。F5b：空菜单不算有菜单，合并条目带 `kinds`（`b63fee0`）；导航未读角标复用现有未读查询（`7273ab0`）；今天页「需要留意」按拍板排布，桌面右栏只放留意、无卡片时不渲染（`f57fa67`）。逐域失效见上方 F5b 备注。typecheck / lint 通过，文案单测 13，新端 202 passed / 4 skipped；四张亮暗截图及无卡片桌面图已复看。已 push，CI `35703106159` 首跑五项全绿，无未改代码重跑。未开始 F6。 |
| F6 ⌘K 动作 | ☑ | `a4f56f9` / `5f79f9f` / `bf91a0f` / `31998b8` / `3fe30c2`（回归补充 `255e331`） | 动作注册表、带 `?create=1` 的现有新建状态、手机底部 sheet 与启用时才出现的「交给小管家」完成。三笔手感修复依次是待办直接认领 `5f79f9f`、三餐直达餐次 `bf91a0f`、日历日期详情居中弹窗 `31998b8`。`31998b8` 的 CI `35809699351` 第 1 次旧端 Web 四条用例停在登录页，第 2 次**未改代码重跑**五项全绿；**待查：未改代码重跑通过，旧端 C3 删除前不追**。`a4f56f9` CI `35812586748` 新端计数型 E2E 失败；`3fe30c2` 改为打开面板才读 agent 状态，CI `35813436159` 首跑五项全绿。`test:unit` 本地 13/13；`255e331` 补「普通成员搜不到管理员动作」「agent 未启用时无交给小管家」E2E，手机 / 桌面 4/4 通过，CI `35830689892` 首跑五项全绿。手机 sheet 的 `.tmp-shots/f6-sheet-390x844-{light,dark}.png` 已逐张复看，无白屏、撞色或横向溢出。F7 尚未开始。 |
| F7 家庭设置 | ☐ | | |
| F8 收口 | ☐ | | |

...
本批按 A7.4-A 执行指令先落地 8 项日常只读能力：

- `get_member_tasks`：按同家庭成员和 `pending | completed | all` 查询任务，单次最多 50 条。
- `get_family_schedule`：复用统一家庭日历聚合，查询未来最多 30 天并保留业务深链接。
- `get_inventory_summary`：结合库存项与正数量批次，区分低库存、7 天内临期和全部库存。
- `get_shopping_list`：兼容原有数组响应，新增 `pending | purchased | all` 状态过滤。
- `search_recipes`：基于现有 `Dish`、个人做法和食材关系搜索；当前模型没有独立菜谱标签，因此 `tags` 对应受控菜品分类。
- `get_dish_plan`：只读查询未来最多 30 天已有菜单及有效点菜项，不隐式创建菜单。
- `get_weather`：通过 OpenWeatherMap 预报 API 查询最多 5 天；未配置密钥或上游不可用时返回稳定错误码，不记录密钥和请求详情。
- `get_member_profile`：只返回同家庭活动成员的姓名、角色、头像、烹饪偏好和非敏感 Agent 档案设置。

8 项能力均注册 MCP Zod schema，继续经过短期 `runId`、运行白名单、活动成员和家庭范围校验，并生成加密结果卡片。返回条数继续受 48 KB 总字节上限保护。

本批明确不新增迁移：新建家庭设置通过 `AGENT_READ_TOOLS` 默认包含新增工具；已有家庭继续保留管理员已保存的 `agent_settings.readToolsEnabled`，需由管理员显式启用新增工具，运行时代码不会静默改写既有选择。原 §10.2 所列批次、投票、来访、维护、媒体等建议工具仍保留为后续扩展，不在 A7.4-A 范围内。

...
23. **显示状态查询失败先保留成功缓存，主动收起与自动空域不能混为一谈。** F4 的模块
    查询由外壳常驻，首次加载骨架、后台保留缓存。2026-09-21 用户修订：失败仍保留
    本家庭成功缓存（含 off），只有没有可用缓存时才放行；不能
    把「没有响应」当成「没有数据」。对照 hasData 表逐域接统一失效，既包括删除，
    也包括媒体连接器重置、采购收货、任务积分、小管家提案等间接写路径。置顶
    记录只按成员存本机，隐藏只过滤显示，不能在刷新时删掉记录。空域 e2e 用独立
    家庭与真实 override 构造，避免被共享演示数据污染；不通过清空种子数据造空态。


24. **日期盘点要追完输入、契约、比较和存储，不能把 TZ 或测试绿灯当口径。**
    F4.5 确认维护表单的本地中午与 API 真实时刻校验冲突；回归填昨天只是避开缺陷，
    不是默认今天已经修好。家庭 timezone 已有，但浏览器本地、固定上海和 UTC 截日
    仍混用；容器 shell date 与 Node Intl 甚至可能不同。纯日期合法性校验 / 加减天数
    内部用 UTC 不等于有 bug，真实时间点按 UTC 截日 / 拼 UTC 日界才需追查。
    后续应固定业务时钟与浏览器时区，并覆盖默认今天、家庭午夜和月界；完整盘点及
    待拍板批次见 [timezone-audit.md](timezone-audit.md)，本次不修时区代码。


25. **家里人看不到的写入，不能靠对方浏览器里的缓存失效。** 访客在公开邀请页提交点菜请求时，查询失效发生在访客自己的客户端，到不了家里人已经打开的今天页。F5b 不为此做推送，今天页用 1 分钟过期和窗口重新聚焦兜底。要让家里人立刻看见，走将来的 `/events` 事件通道，而不是在公开页上假装刷新了家里的屏幕。

26. **面板未打开不预读小管家状态。** F6 搜索面板曾在关闭时读取 `agent/status`，
    冷启动凭空多出一次请求；CI `35812586748` 的计数型新端 E2E 抓到了这个回归。
    `3fe30c2` 将状态查询限定为打开搜索后再启用，CI `35813436159` 五项全绿。
    以后给低频浮层添数据源，先核对 `enabled` 与触发时机；页面能显示并不代表
    网络请求数量正确，计数型 E2E 要覆盖未打开时为零、打开后才读取。

...
12. **`await` 之后再清空输入框是一个真实的竞态。** 69 个 Playwright 用例第一次在 CI 里真正跑完（51 通过 / 1 失败 / 21 跳过，9.8 分钟），唯一的红是 `agent-ui.spec.ts` 里发送按钮永远禁用、180 秒超时。根因在客户端：`newConversation()` 先 `await createConversation.mutateAsync()` 再 `setDraft('')`，而输入框的 `editable` 和发送按钮的 `disabled` 都看 `busy`。请求一落地，React Query 先把 `isPending` 翻成 false（输入框此时已可编辑），`setDraft('')` 还在 then 的后半段等着执行——这一帧里敲进去的字随后被清掉，于是"输入框有字、发送按钮永远灰着"。修法是把 `setDraft('')` 提到 `await` 之前。真实用户在新建会话时抢着打字会踩同一个坑，只是概率低、没人报过。

    同一个用例还有个测试自身的问题：它用 `isVisible()`（不等待）判断运行时可用与否来决定要不要跑核心断言，于是**快的机器跳过、慢的机器走进来**，两边结论不一样——本地绿了半年，一进 CI 就露馅。凡是"按当前状态决定跑不跑"的分支，前面必须有一条真正会等的断言（这里用 `expect(input).toBeEditable()` 等状态查询落地），否则跳过和通过在报告里长得一模一样。

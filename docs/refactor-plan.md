...
18. **启动台的状态行只读缓存，不能为了每个图块挂一个业务查询。** Phase F2 用
    `useSyncExternalStore` 订阅 QueryCache，快照返回稳定字符串；没有成功缓存就不渲染，
    成功的空数组才可以显示零。列表有服务端上限时用「已载入」说明口径，不冒充家庭总数。
    导航意图触发的留意源预取与状态派生分开，管理员查询按当前成员过滤；用 e2e 同时断言
    **冷开 `/home` 不逐域发请求、预取完成会更新状态、回访缓存不补发列表、普通成员不拉管理数据**。

...
- 旧客户端的 `family-navigation.spec.ts:1238`（「浏览器端已编辑任务…」那句断言）在 CI 上**约一半的概率红**：
  strict mode 说同一段备注文案匹配到 2 个元素。本机隔离跑过一次是绿的，三次 CI 里两次红一次绿。
...
| B7a 资产：列表 + 详情（看） | ☑ | | `/house/assets` 卡片墙（使用中/全部 + 分类筛选）+ 右栏「临近事项」（30 天内的维护、14 天内的续费；**逾期的续费旧页面漏掉了，这里补上**）；`/house/assets/:id` 档案、保修/续费状态卡（订阅可「标记已续费」）、维护计划与维护记录只读、凭证与资料（打开走 `/access` 换 60 秒签名地址，先开空标签页再填地址，否则被弹窗拦截）、编辑档案、停用/恢复 |
| B7b 资产：维护与资料（改） | ☑ | | 维护计划新增/停用/启用、耗材关联增删改、完成维护（耗材预览三态、缺口加入某天购物清单、可选扣库、幂等键一次挂载一个，失败不关弹窗好让人用同一个键重试）、资料外链或上传（图片/PDF ≤10MB）、删除资料。顺手把 `lib/api.ts` 的 `uploadPhoto` 抽成通用的 `postForm`。**至此 B7「资产」整段完成** |
| B8a 财务：概览 + 流水 + 记一笔 | ☑ | | `/house/finance` 月份步进 + 概览/流水/账户三视图；记一笔（支出/收入/转账）、撤销（另记一笔反向流水）、账户增改与启停。**照旧页改了四处**：金额负号不再被 `Math.abs` 吃掉（超支的「剩余」以前显示成正数）、两位小数改用字符串正则判（`8.29*100` 是 828.999…，浮点算会误拒合法金额）、幂等键改成一次弹窗一个（旧的每次点击都换新键，等于没有幂等）、翻到往月记账时日期默认落在那个月 1 号并提示记到了哪个月 |
| B8b 财务：预算 + 收支分类 | ☑ | | 预算视图按支出分类设/改/删（新建时不带 expectedVersion，带了后端会判成「状态已变化」），概览补上预算进度条，账户视图右栏是收支分类的新增与启停。**旧页面的一个坑顺手堵上**：分类停用后它的预算在预算页会消失、却仍在概览里算着，这版把这类预算也列出来，好歹能删。**至此 B8「财务」整段完成** |
| B9 知识库 | ☑ | | `/house/knowledge` 卡片墙 + 搜索（回车或按钮提交，不是边打边搜）+ 使用中/已归档 + 分类筛选；详情弹窗里看正文（纯文本，`whitespace-pre-wrap` 还原换行，不做 markdown）、版本历史、还原到某一版、归档/恢复。写操作都带 expectedVersion 和幂等键（一次弹窗一个）。**顺手补上旧页面漏掉的深链**：小管家给的 `/knowledge?articleId=` 以前被无声忽略，这版会直接把那篇打开。知识库没有附件，只有一个参考链接字段 |
| B10 回忆 | ☑ | | `/house/memories` 卡片墙（封面是最早传的那张照片）+ 搜索 + 珍藏中/已归档 + 类别筛选；详情弹窗里看照片、故事、标签、来源链接，能加照片（最多 6 张、≤10MB、GIF/JPEG/PNG/WebP）、编辑、归档/恢复。照片正文是公开签名地址（10 分钟有效），`<img src="/api…">` 直接用，不用带令牌；但每次响应都重签，地址每次都变，浏览器缓存基本不生效——这是后端的取舍，先照搬。**顺手改了契约里一处笔误**：注释说 multipart 文件字段是 `photo`，控制器其实是 `FileInterceptor('file')` |
| B11a 出行：行程 + 清单 | ☑ | | `/house/travel` 四个状态分段的行程卡片，`/house/travel/:id` 是详情：清单勾选/跳过/改/移除、加一项、状态流转（完成/取消/重新打开/归档/恢复）、编辑行程、跳去设置提醒。**照旧页改了三处**：完成行程按钮在还有待处理项时直接禁用并说明原因（旧页面让人点了才吃 409）、新清单项的 sortOrder 用现有最大值 +1（旧的用 `items.length`，移除过项之后会撞号、排序乱掉）、「移除」只对能管的人显示（后端本来就只让创建者和管理员移除）。日期补上了年份 |
| B11b 出行：打包模板 | ☑ | | `/house/travel` 加一层「行程 / 打包模板」视图：模板列表（含归档的）、新建/编辑（items 整体替换）、归档/恢复；行程详情里「套用模板」能挑一个塞进清单，已经用过的在挑选列表里置灰。**按钮按 `canManage` 显示**——后端应用模板要求是创建者或管理员，旧页面按 `canEditChecklist` 显示，普通成员点了才吃 403。**至此 B11「出行」整段完成** |
| B12 备份 | ☑ | | `/house/backups`（只有管理员能看，普通成员给一句说明就停住）：worker 在线状态、存储用量、最近一次备份、进行中的任务、运行历史 60 条；能排「立即备份」「查容量」「恢复演练」，排队中的任务能取消，还能改计划与保留策略。**这一页没有下载、没有删除备份、也没有真恢复**——界面上的「恢复演练」是拿某次备份在临时库里还原一遍再删掉，不碰正在用的库；真灾难恢复是运维跑 `scripts/restore-prod.sh`。历史里的演练按钮也判了 workerOnline（旧页面只有顶部那个判了，离线时从历史排的任务没人领）。策略表单用 `key={policy.id}`，不能用 updatedAt——看板每 10 秒轮询一次，按 updatedAt 重挂载会把人正在填的表单冲掉（这个坑是写完用例才发现的）|
| B13 家庭动态 | ☑ | | `/me/activity` 按天分组的时间线（今天/昨天/日期），全部 / 成员 / 菜单三档。每行的 `targetPath` 还是后端写的旧路径：搬过来的用 `lib/routes.ts` 的 `toNewRoute` 转成新路径走软导航，没搬的给一个 8088 的旧版链接在新标签打开——不留点不动的行 |
| B14a 观影：概览 + 片库 | ☑ | | `/eat/media` 是入口：三格统计、接下来排了哪几部、媒体服务状态、去各处的入口；`/eat/media/library` 是片库（筛选/搜索/翻页、详情弹窗、播放、加进片单、管理员同步）。片单/观看记录/观影设置还没搬，入口先指旧版并注明。海报和播放地址可能是绝对 URL 也可能是后端签的相对路径（24 小时有效），相对的补 `/api`；加载失败退成占位图，不弹错。片库回归用 mock（隔离库里 Plex/Emby 是空配置）。**顺手补了个洞**：认不出来的分段以前渲染成白屏，现在给一句「没有这一页」+ 回今天。**注意**：`nav.ts` 里已经没有 `legacy:` 分段了，桥接页只剩「认不出来」这一个分支 |
| B14b 观影：片单 + 观影投票 | ☑ | | `/eat/media/watchlist`：状态筛选 + 搜索、在线搜片（豆瓣/TMDB/Bangumi 三源各查各的，没配就只有那条显示未配置）或手动加、改安排（状态流转按后端允许的来，排期和状态互相牵着）、移出、Plex/Emby 里有的直接给播放入口、MoviePilot 订阅（提交/刷新/取消/补 TMDB ID）、单片投票和「挑几部投票」。**旧页 3103 行拆成了 6 个文件**（页 + 卡片 + 表单 + 搜索面板 + 投票弹窗 + 订阅相关弹窗）。用例覆盖：手动加一部 → 发起投票 → 结束投票 → 改成已排期（顺序不能反：排期之后投票入口就没了，投票期间后端也不让改状态）|
...
| D1 AI 找菜谱 | ☐ | | 需用户确认范围 |
| D2 换栈决策门 | ☐ | | 需用户决定 |
| E0 Home Assistant 接入设计 | ☑ | | 设计稿在 `docs/home-assistant-plan.md`，等 King 拍板三件事：HA 装哪、门锁安防要不要进来、今天页放哪几个设备 |
| E1 智能家居：连接器 + 白名单（只读） | ☐ | | 可与 C2 试用期并行（只动 `apps/web` 和 API） |
| E2 智能家居：控制 | ☐ | | 建议等 C3 删完旧客户端再做 |
| E3 智能家居：HA → 小管家（webhook 联动） | ☐ | | |
| E4 智能家居：小管家 → HA（日程/家务触发场景） | ☐ | | |
| E5 智能家居：今天页卡片 + 收口 | ☐ | | |

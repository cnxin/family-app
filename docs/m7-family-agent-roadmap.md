...
> 文档状态：M7-A4.1 与 M7-A4.2 已实施，M7-A4.3 起待后续批次
> 更新日期：2026-08-04
> 目标分支：`uitest`
...
仅在消息渠道批次增加：家庭、成员、平台、外部账号摘要、配对时间、最后使用时间、撤销时间和版本。不得保存外部平台密码或机器人 Token。
...
- 增加提案表、状态机、预计变化和确认卡。
- 首批接入任务、提醒、投票、菜单和购物清单。
- 确认时复用现有权限、事务、幂等和来源版本。
- 对已有撤销能力的业务显示撤销入口；没有安全撤销契约的操作不伪造撤销。

### M7-A4.4 消息渠道

- 增加一次性成员配对、撤销、失效和审计。
- 首批只读查询；写提案跳转回 Family App 确认。
- 不将消息平台账号自动视为家庭成员，不共享访客邀请或媒体连接器凭据。
...
7. M7-A4.4/A4.5：真实消息渠道与 Docker 生产加固。

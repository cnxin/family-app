...
环境变量或只读密钥文件作为服务器默认配置。`owner/admin` 也可从“家庭观影 -> 数据源设置”建立家庭级覆盖，普通成员不能读取或修改设置。家庭 Token 使用独立 `INTEGRATION_SECRET_KEY`（或 `_FILE`）进行 AES-256-GCM 加密后保存；API 和前端只返回是否已配置及末尾四位提示，不回显明文或密文。删除家庭覆盖后恢复服务器默认配置。

搜索按当前登录家庭动态解析配置，缓存键包含家庭与配置版本；修改或恢复设置会清除该家庭搜索缓存。不同家庭的地址、启停状态与凭据相互隔离。活动只记录数据源名称和操作，不记录地址、Token 或 API Key。国内部署可将三个 API 地址指向自有合规出口或缓存代理。

## 6. 自动验收

- 提供方契约使用固定模拟响应覆盖 TMDB Bearer 认证、豆瓣桥接 Token、Bangumi `User-Agent` 与动画类型过滤。
- 契约测试覆盖三源合并、5 分钟缓存、单源失败和未配置状态，并断言响应不包含 Token 或服务地址。
- API 回归覆盖登录要求、空关键词、无配置/离线降级、管理员权限、家庭隔离、密文落库、凭据不回显和缓存失效。
- Playwright 在 390 x 844 与 1440 x 900 视口覆盖鼠标打开搜索、三源状态、选择候选、返回搜索、手动录入和数据源设置。

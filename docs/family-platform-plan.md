...
当前过渡模型已经将家庭权限角色统一为 `owner/admin/member`，并把 `prefersCooking` 作为独立偏好。后续拆分账号与家庭成员表时，该偏好迁入 `member_preferences`，不再把“谁经常做饭”编码成权限角色。
...
M2-D 前端质量批次已于 2026-07-28 完成：Expo 官方 ESLint 规则已经接入；Playwright 使用本机 Chrome 固化一次鼠标登录，并复用会话验证 390 x 844 移动视口与 1440 x 900 桌面视口。回归覆盖首页、点菜、菜单历史、采购库存和个人页，不执行点菜、状态、偏好或库存修改，同时检查控制台异常和页面横向溢出。

M2 最后一批继续聚焦会话续期与撤销边界、请求级日志和正式部署配置收口。
...
1. 明确账号与家庭成员的长期边界，补充安全续期、登出撤销和角色变化后的会话策略。
2. 为 API 增加请求 ID、结构化错误日志和不含敏感信息的家庭上下文。
3. 整理正式家庭部署的密钥、CORS、HTTPS、健康检查和备份配置模板。

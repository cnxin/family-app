...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2、M3 已冻结，M4 的观影流程和 M5 的访客流程已经完成；M6-A1 已交付库存食材关联、采购差额快照与前端明细，详见 [库存与采购差额验收](m6-inventory-shopping-acceptance.md)。下一次实施范围：

1. 为库存增加不可变流水和幂等来源键，再实现采购项明确确认入库、菜单明确确认扣库及撤销操作。
2. 在 NAS 的 Plex 中配置真实播放 Webhook，验证 Mac API 的局域网可达性、来源 IP 和已映射账号；Emby 部署后复用同一流程。
3. 待中兴设备型号确认后，评估只读网络连接器；家庭资产、维护周期和文档作为下一项独立家庭运营模块。

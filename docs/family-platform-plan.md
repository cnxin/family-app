...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-07-29
> 当前版本：点菜、多人厨房协作、菜谱、采购库存、统一日历、家庭任务、家庭投票、可配置提醒与站内通知已进入本地 Web 演示阶段
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2 已冻结，M3-A 统一日历、M3-B 家庭任务/通知、M3-C 通用投票与 M3-D 可配置提醒已经完成；后续仍不同时启动观影或访客 UI：

1. 补齐活动记录和成员管理界面，形成 M3 家庭公共能力闭环。
2. 在观影 MVP 中通过来源关联复用家庭投票和提醒，不另建一套规则。
3. 观影连接器继续等待 Plex/Emby 主播放来源和 MoviePilot API 条件确认。

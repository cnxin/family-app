...
| 家庭资产 | 已实现维护与安全资料 | 家电、购买保修资料、维护周期、不可变历史、日历、提醒、审计与短时签名附件；耗材关联后续建设 |
...
M6-A3 已交付 `home_assets`、`asset_documents`、`maintenance_plans` 和不可变 `maintenance_records`。资产保留购买、保修和位置等主档，图片上传与 HTTP(S) 资料链接复用现有附件能力；维护完成使用计划级事务锁和家庭幂等键，从实际执行日推进下次周期，并取消旧周期提醒。资产归档、计划停用和历史保留均有明确状态边界。详细契约见 [M6 家庭资产与维护验收](m6-assets-maintenance-acceptance.md)。`consumable_links` 尚未建设，后续必须显式关联库存和单位，不能按名称猜测耗材。
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2、M3 已冻结，M4 的观影流程和 M5 的访客流程已经完成；M6-A1/A2 已交付库存采购闭环，M6-A3 及其收尾批次已交付家庭资产、维护周期、统一日历、活动审计与安全附件，M6-A4 已交付积分流水、任务积分、家庭奖励与兑换状态机。当前共有 35 个有序迁移，详见 [M6 库存与采购闭环验收](m6-inventory-shopping-acceptance.md)、[M6 家庭资产与维护验收](m6-assets-maintenance-acceptance.md) 与 [M6 积分与奖励验收](m6-points-rewards-acceptance.md)。下一次实施范围：

1. 将资产维护耗材显式关联库存和购物清单，不根据名称猜测对应关系。
2. 增加外部通知渠道、成员通知偏好、投递记录与失败重试。
3. 增加备份计划、保留策略、容量告警和恢复状态；继续使用独立恢复库演练，不覆盖运行中数据。
4. 待中兴设备型号确认后评估只读网络连接器；真实 NAS Webhook 联调作为部署验收，不把密钥写入代码或文档。

...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2 已冻结，下一次代码迭代进入 M3，仍不同时启动观影或访客 UI：

1. 建立统一日历聚合 API，让菜单、任务和自定义事件使用同一展示模型但保留各自业务表。
2. 落地家庭任务、周期实例和通用通知模型。
3. 落地家庭投票基础模型，为活动、点菜和后续观影复用。

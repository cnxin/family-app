...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-07-29
> 当前版本：点菜、多人厨房协作、菜谱、采购库存、统一日历、家庭任务、家庭投票与站内通知已进入本地 Web 演示阶段
...
- `menu_events`：点菜、认领、状态变化、主厨变化和菜单完成的不可变审计记录；需要提醒时保存接收成员与已读时间。
...
当前基础版支持整组改票、撤票、手动结束、重新开启和软删除。已有选票后锁定候选项与选择规则，但仍允许维护标题、说明、分类和未来截止时间。投票结束后不允许改票，因此结果直接由不可变选票计算，不额外维护容易漂移的 `poll_results`；未来只有在外部审计或规则变更确有需要时才增加关闭快照。`sourceModule` 与 `sourceId` 已为点菜、观影和采购等业务预留关联，截止提醒由后续通用提醒模块接入。详细契约见 [M3-C 通用家庭投票验收](m3-polls-acceptance.md)。
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2 已冻结，M3-A 统一日历、M3-B 家庭任务/通知与 M3-C 通用投票已经完成；后续仍不同时启动观影或访客 UI：

1. 为菜单、任务、家庭事件与投票增加可配置提醒，继续保持来源业务表为权威数据源。
2. 补齐活动记录和成员管理界面，形成 M3 家庭公共能力闭环。
3. 在观影 MVP 中通过来源关联复用家庭投票，不另建一套投票规则。

M3 通过验收后，再进入观影 MVP；访客系统继续使用独立临时权限模型。

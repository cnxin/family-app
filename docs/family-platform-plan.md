...
M2 剩余批次继续聚焦请求级日志、正式部署配置和账号与家庭成员长期边界的收口。
...
1. 为 API 增加请求 ID、结构化错误日志和不含敏感信息的家庭上下文。
2. 整理正式家庭部署的密钥、CORS、HTTPS、健康检查和备份配置模板。
3. 明确独立账号与家庭成员的迁移路径，不让登录凭据继续绑定成员展示资料。
4. 完成 M2 验收清单并冻结数据库基础约定。

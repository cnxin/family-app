...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-07-29
> 当前版本：点菜、多人厨房协作、菜谱、采购库存与统一日历已进入本地 Web 演示阶段
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2 已冻结，M3-A 统一日历已经完成；后续仍不同时启动观影或访客 UI：

1. 落地家庭任务、周期实例和通用通知模型，并接入现有统一日历展示模型。
2. 落地家庭投票基础模型，为活动、点菜和后续观影复用。
3. 为菜单、任务与家庭事件增加可配置提醒，继续保持来源业务表为权威数据源。

...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-07-29
> 当前版本：点菜、多人厨房协作、菜谱、采购库存、统一日历、家庭任务与站内通知已进入本地 Web 演示阶段
...
| 任务与投票 | 任务已实现，投票待建设 | 家务、家庭决策、周期任务、完成记录 |
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2 已冻结，M3-A 统一日历与 M3-B 家庭任务/通知已经完成；后续仍不同时启动观影或访客 UI：

1. 落地家庭投票基础模型，为活动、点菜和后续观影复用。
2. 为菜单、任务与家庭事件增加可配置提醒，继续保持来源业务表为权威数据源。
3. 补齐活动记录和成员管理界面，形成 M3 家庭公共能力闭环。

...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2、M3 已冻结，M4 的片单、三源搜索、单片/多片投票、订阅请求、媒体连接设置、MoviePilot 整理完成通知和 Plex/Emby 用户映射已经完成。下一次实施范围：

1. 使用轮换后的 Plex Token 和 MoviePilot API Key 完成 NAS 真实数据只读验收；Emby 部署后复用同一契约验收。
2. 接入 Plex/Emby 播放开始、停止和进度事件，生成可追溯的观看会话与成员进度；未映射用户的播放事件只审计，不归到任何成员。
3. 验证重复、乱序、离线补报和服务器切换事件，沿用事件白名单、独立密钥、来源限制、幂等和脱敏策略。

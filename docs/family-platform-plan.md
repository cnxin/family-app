...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-08-01
> 当前版本：点菜、多人厨房协作、菜谱、采购库存流水、家庭资产维护、统一日历、活动审计、安全附件、家庭观影与访客协作已进入本地 Web 演示阶段
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2、M3 已冻结，M4 的观影流程和 M5 的访客流程已经完成；M6-A1/A2 已交付库存采购闭环，M6-A3 及其收尾批次已交付家庭资产、维护周期、统一日历、活动审计与安全附件，当前共有 33 个有序迁移，详见 [M6 库存与采购闭环验收](m6-inventory-shopping-acceptance.md) 与 [M6 家庭资产与维护验收](m6-assets-maintenance-acceptance.md)。下一次实施范围：

1. 建设通用积分流水、家庭奖励、兑换审批与撤销，保持与任务、成员权限和活动记录的明确边界。
2. 将资产维护耗材显式关联库存和购物清单，不根据名称猜测对应关系。
3. 增加外部通知渠道、成员通知偏好、投递记录与失败重试。
4. 增加备份计划、保留策略、容量告警和恢复状态；继续使用独立恢复库演练，不覆盖运行中数据。
5. 待中兴设备型号确认后评估只读网络连接器；真实 NAS Webhook 联调作为部署验收，不把密钥写入代码或文档。

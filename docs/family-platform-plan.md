...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-08-01
> 当前版本：点菜、多人厨房协作、菜谱、采购库存流水、家庭资产维护、积分奖励、统一日历、活动审计、安全附件、家庭观影与访客协作已进入本地 Web 演示阶段
...
前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 `corepack pnpm test:web` 重复执行。M2、M3 已冻结，M4 的观影流程和 M5 的访客流程已经完成；M6-A1/A2 已交付库存采购闭环，M6-A3/A5 已交付家庭资产、维护周期、耗材库存采购、统一日历、活动审计与安全附件，M6-A4 已交付积分流水、任务积分、家庭奖励与兑换状态机。当前共有 36 个有序迁移，详见 [M6 库存与采购闭环验收](m6-inventory-shopping-acceptance.md)、[M6 家庭资产与维护验收](m6-assets-maintenance-acceptance.md) 与 [M6 积分与奖励验收](m6-points-rewards-acceptance.md)。下一次实施范围：

1. 增加外部通知渠道、成员通知偏好、投递记录与失败重试。
2. 增加备份计划、保留策略、容量告警和恢复状态；继续使用独立恢复库演练，不覆盖运行中数据。
3. 待中兴设备型号确认后评估只读网络连接器；真实 NAS Webhook 联调作为部署验收，不把密钥写入代码或文档。

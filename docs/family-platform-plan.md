...
> 文档状态：总体方向已确认，按阶段实施
> 更新日期：2026-08-03
> 当前版本：点菜、多人厨房协作、菜谱、采购库存流水、家庭资产维护、积分奖励、外部通知、备份运维、家庭知识库、家庭出行、家庭回忆、今日家庭助理、统一日历、活动审计、安全附件、家庭观影与访客协作已进入本地 Web 演示阶段；家庭智能体只读基础与 App 内“问问小管家”已进入验收
...
- 去哪里：家庭行程、出行分工、打包清单和可复用模板。
- 记住什么：非敏感家庭回忆、私有照片和既有家庭事项来源。
- 怎么问：通过受控家庭智能体查询现有家庭数据、生成操作提案并由成员确认。
...
9. **智能体不是业务权威**：模型只负责理解和编排，家庭数据、权限、确认、事务与审计继续由本系统负责。
...
| 购物清单 | 已实现库存差额与确认入库 | 手动数量、菜单生成、库存差额、确认入库和来源快照 |
| 家庭库存 | 已实现采购与菜单闭环 | 低库存提醒、采购差额、确认入库、菜单确认扣库与不可变流水 |
| 家庭日历 | 已实现七类来源统一聚合 | 菜单、任务、观影、来访、维护、出行与家庭事件 |
| 任务与投票 | 已实现 | 家务、家庭决策、周期任务、完成记录与成员通知 |
| 可配置提醒 | 已实现 | 菜单、任务发生日、家庭事件、投票、资产维护与出行的多人定时提醒 |
| 通知中心 | 已实现站内与外部投递 | 站内通知、Webhook/ntfy、成员模块偏好、尝试历史、自动与人工重试 |
| 积分与奖励 | 已实现 | 通用不可变积分流水、任务积分、家庭奖励、兑换审批和撤销，不只服务儿童 |
| 观影 | 已实现家庭与 NAS 闭环 | 片单、投票、排期、订阅、媒体同步、用户映射与观看记录 |
| 访客 | 已实现独立协作流程 | 临时邀请、有限权限、访客 Wi-Fi、观影投票、点菜和匿名化 |
| 家庭资产 | 已实现维护、耗材与安全资料 | 家电、购买保修资料、维护周期、显式耗材库存、采购缺口、不可变历史、日历、提醒、审计与短时签名附件 |
| 系统备份 | 已实现计划、保留、告警与恢复演练 | 独立 worker、完整备份、容量状态和隔离恢复验证 |
| 家庭知识库 | 已实现文本资料与版本历史 | 家庭流程、设备说明、常用联系、检索、置顶、归档和不可变版本快照 |
| 家庭出行 | 已实现计划与协作清单 | 行程日期、准备进度、成员分工、完成或跳过、可复用打包模板、日历与提醒 |
| 家庭回忆 | 已实现首批时间线 | 非敏感故事、标签、私有照片、来源回链、归档和不可变操作历史 |
| 家庭智能体 | 本机集成与交互增强已交付 | Hermes 可替换运行时、15 个只读/提案 MCP、用户确认、加密结果卡片、重试、审计与故障降级 |
...
M6-A2 已增加不可变 `inventory_transactions`。入库、消耗、数量调整和撤销均保存家庭、库存项、变化前后数量、变化量、单位、操作者、来源及时间；数据库触发器拒绝更新或删除历史流水。购物项只有在用户勾选并确认预计变化后才入库，自由名称项必须显式选择同单位库存项；菜单结束本身不扣库存，用户需在菜单锁定后查看逐项预览并确认。菜单扣库按整餐操作组提交，库存不足或已有同食材其他单位时整组拒绝；撤销通过最近操作组的反向流水恢复，重复请求由家庭范围幂等键收敛。详细契约见 [M6 库存与采购闭环验收](m6-inventory-shopping-acceptance.md)。
...
M6-A4 已交付通用积分模块。`points_accounts` 是使用事务行锁维护的余额缓存，`points_ledger` 是数据库触发器保护的不可变事实；手工发放、任务完成和兑换申请使用明确来源与幂等键。兑换在申请时先扣分防止并发超兑，拒绝、申请人取消和管理员撤销通过唯一反向流水退回。只有家庭管理员能调整积分、维护奖励和设置任务积分，普通成员可查看余额与流水、兑换启用奖励并管理自己的待确认申请。迁移 35 只规范化开发热重载期间曾执行的迁移 34 首稿约束，不改业务行。详细契约见 [M6 积分与奖励验收](m6-points-rewards-acceptance.md)。
...
M5-D 已支持访客档案匿名化。管理员仅能在没有已安排来访时执行，操作会移除身份字段、撤销仍有效链接并去标识化带访客 ID 的活动摘要；来访、点菜和投票历史保留匿名事实。详细边界见 [M5-D 访客匿名化验收](m5-guest-anonymization-acceptance.md)。
...
- `maintenance_consumables`：维护计划与库存项的显式关联、单次用量和单位快照。

M6-A3/A5 已交付 `home_assets`、`asset_documents`、`maintenance_plans`、`maintenance_consumables` 和不可变 `maintenance_records`。资产保留购买、保修和位置等主档，图片上传与 HTTP(S) 资料链接复用现有附件能力；维护耗材按库存项 ID、数量和单位显式关联，完成前显示逐项库存变化，只有用户选择“同时确认扣库”才写入不可变库存流水。库存不足可将准确缺口幂等加入指定日期购物清单，购物项保留耗材和库存引用，购买后直接进入原库存项。维护完成继续使用计划级事务锁和家庭幂等键，从实际执行日推进下次周期，并取消旧周期提醒；选择跳过扣库时也保存耗材快照。详细契约见 [M6 家庭资产与维护验收](m6-assets-maintenance-acceptance.md)。

M7-A1 已交付家庭知识库。`knowledge_articles` 保存当前可读版本，`knowledge_article_revisions` 为数据库触发器保护的不可变完整快照；创建、编辑、置顶、归档、恢复和历史还原都使用家庭幂等键与预期版本，避免重复点击和并发覆盖。所有成员可阅读和创建，创建者维护自己的文章，`owner/admin` 可维护全部并设置家庭置顶。知识库当前只接受文本、标签和 HTTP(S) 参考链接，不上传附件，不在活动审计中复制正文。详细契约见 [M7 家庭知识库验收](m7-household-knowledge-acceptance.md)。

身份证件、健康数据、账号密码和连接器密钥等敏感内容仍不纳入知识库；这些内容需要独立的字段级加密、细粒度授权和审计策略，不能仅依赖普通文章权限。

M7-A2 已交付家庭出行计划、协作清单和打包模板。行程只保存名称、城市或区域、日期和普通备注；清单支持分类、数量、成员分工、完成、恢复、跳过和移除，计划完成前必须显式处理全部待办。模板按版本保存并在同一行程中最多应用一次。所有写操作使用家庭幂等键、请求指纹、预期版本和事务行锁；`travel_operations` 由数据库触发器保护为不可变记录。完成、取消和归档会取消未发送的出行提醒，重开不会静默恢复提醒。出行进入首页、统一日历、提醒中心和家庭活动流，详细契约见 [M7 家庭出行验收](m7-household-travel-acceptance.md)。

出行模块不保存证件号码、票据内容、支付信息、健康资料或精确轨迹。未来如需票据或证件能力，必须采用独立加密字段、细粒度授权和访问审计，不放入普通备注或清单字段。

M7-A3 已交付家庭回忆首批与普通成员“今日家庭助理”。`family_memories` 保存当前回忆，`family_memory_photos` 使用家庭私有目录和短时 HMAC 签名读取，`family_memory_operations` 由数据库触发器保护为不可变操作记录。创建者或家庭管理员可编辑、归档、恢复和追加照片；所有写操作使用家庭幂等键，编辑和状态变化使用预期版本及事务行锁。首页助理只查询并汇总已有菜单、任务、提醒、购物、来访、资产维护、出行和最近回忆，不保存重复事实。详细契约见 [M7 家庭回忆验收](m7-family-memories-acceptance.md)。

家庭回忆首批不做人脸识别、精确定位或公开分享，不保存证件、健康、支付、账号凭据和连接器密钥。未来如增加人物或地点能力，必须先单独评估同意、保留期限、撤回和细粒度访问审计。
...
M6-A6 已交付加密的 Webhook/ntfy 家庭通知渠道、成员独立模块偏好、逻辑投递与不可变尝试历史。外部投递采用稳定幂等键、短事务领取、超时和指数退避；永久失败由成员或管理员明确重试，任何外部故障都不回滚站内通知和来源业务。迁移前历史不会在升级后补发，删除渠道也不会删除投递快照。详细契约见 [M6 外部通知渠道与投递验收](m6-external-notifications-acceptance.md)。

### 15.1 家庭智能体运行时

M7-A4 计划把 Hermes 作为可替换的独立 Agent 运行时接入。Hermes 使用独立容器、专用数据卷和受限网络，不加入数据库网络，不挂载 Docker Socket、项目源码、上传卷或备份目录。Family App 通过内部运行时接口调用 Hermes，Hermes 只通过短时成员委托和白名单 MCP 工具访问 NestJS API；模型不能自行决定家庭、成员或权限。

现有“今日家庭助理”继续作为不依赖模型的确定性摘要。智能体首批只开放日历、菜单、库存、知识库、出行、观影和回忆查询；任务、提醒、投票、菜单和购物等写操作必须先生成结构化提案，由用户查看预计变化并使用鼠标或触控确认。Hermes 或模型不可用时不影响任何核心家庭功能。完整架构、数据模型、Docker 边界、测试矩阵和后续生活能力见 [M7-A4 家庭智能体与后续能力实施方案](m7-family-agent-roadmap.md)。

### 15.2 家庭财务

M8-A1 已交付独立家庭财务模块：家庭账户、默认与自定义收支分类、收入、支出、账户间转账、月度分类预算和反向撤销。交易与 posting 由数据库触发器保护为不可变历史，余额按初始余额和 posting 实时计算；普通成员可查看共享账本并记账，只有管理员可管理账户、预算和撤销。Hermes 只通过 `get_finance_summary` 查询真实数据，通过 `propose_finance_transaction` 生成单笔待确认提案，不能直接写账，也不把财务事实写入长期记忆。完整边界与后续导入路线见 [M8 家庭财务与 Hermes 记账方案](m8-family-finance-plan.md)。
...
M6-A7 已交付家庭范围的备份策略与运行状态、独立备份 worker、自动保留、容量告警和隔离恢复演练。API 只负责权限、排队、调度和状态，不持有 Docker socket，也不返回备份路径或内容；worker 只读挂载上传卷并写入专用备份目录。完整备份包含数据库、附件、清单和 SHA-256 校验和；恢复演练只创建随机临时数据库，运行待执行迁移并解压附件验证，完成或失败后都清理临时环境。生产灾难恢复另有拒绝覆盖活动数据库的脚本。详细契约见 [M6 备份运维验收](m6-backup-operations-acceptance.md)。
...
- [x] 库存关联食材与采购自动差额计算。
- [x] 采购确认入库、菜单确认扣库与不可变库存流水。
- [x] 家庭资产、维护周期、购买凭证和说明文档。
- [x] 资产维护进入统一日历、活动审计和家庭安全附件访问。
- [x] 积分流水、奖励、兑换、审批与撤销。
- [x] 资产耗材与库存、购物清单的显式关联。
- [x] 外部通知渠道、成员偏好与失败重试。
- [x] 备份计划、保留策略、容量告警与恢复状态。

### M7：按真实需求扩展

- [x] 家庭知识库、不可变版本历史与并发编辑保护。
- [x] 非敏感家庭出行计划、协作清单与打包模板。
- [x] 非敏感家庭回忆、私有照片与今日家庭助理。
- [x] Hermes 独立运行时、只读家庭 MCP 与 App 内“问问小管家”。
- [x] 任务、提醒、投票、菜单和购物的 Agent 操作提案与明确确认。
- [x] 普通成员今日家庭工作台、家庭收件箱与触控快捷新增。
- [ ] 纪念日、共享相册和“往年今天”。
- [x] 食品批次、保质期、临期状态、FIFO 扣库预览和智能菜单候选。
- [ ] 访客点菜采纳、家庭周报和知识库附件。
- [ ] PWA 弱网能力与 Home Assistant 受控连接器。
- 儿童相关能力。
- [x] 家庭共享账户、不可变收支流水、月度预算与 Hermes 确认式记账。
- 家庭健康。

未完成模块只有在数据安全、权限和使用场景明确后进入实施。

## 22. 下一次实施范围

前端质量门禁可以通过 `corepack pnpm lint`、`corepack pnpm typecheck` 和 Expo Web 导出重复执行。M2、M3 已冻结，M4 的观影流程和 M5 的访客流程已经完成；M6-A1/A2 已交付库存采购闭环，M6-A3/A5 已交付家庭资产、维护周期、耗材库存采购、统一日历、活动审计与安全附件，M6-A4 已交付积分流水、任务积分、家庭奖励与兑换状态机，M6-A6 已交付外部通知渠道、成员偏好、投递历史与失败重试，M6-A7 已交付备份计划、保留策略、容量告警和隔离恢复演练，M7-A1 已交付家庭知识库与不可变版本历史，M7-A2 已交付非敏感家庭出行计划、协作清单和打包模板，M7-A3 已交付非敏感家庭回忆、私有照片和今日家庭工作台，M7-A4.1 至 A4.6 已交付受控家庭智能体能力，M7-A5.2 已交付食品批次、临期状态、FIFO 扣库预览和可解释智能菜单，M8-A1 已交付家庭共享账本、预算和 Hermes 确认式记账；普通成员首页已进一步精简为默认行动视图，次要内容通过点击展开。当前共有 58 个有序迁移，详见 [M7 家庭智能体首批验收](m7-family-agent-acceptance.md)、[M8 家庭财务与 Hermes 记账方案](m8-family-finance-plan.md) 和 [M7-A5.2 食品批次、保质期与智能菜单验收](m7-food-batches-smart-menu-acceptance.md)。下一次实施范围：

1. M7-A4.7 完成 Hermes 生产容器网络、出口允许列表、资源限制和 NAS 部署验收。
2. 后续实施纪念日与共享相册、访客采纳与家庭周报、知识库附件，再评估消息渠道、PWA 离线和 Home Assistant。
3. 待中兴设备型号确认后评估只读网络连接器；真实 NAS Webhook 和 Hermes 模型/消息渠道联调作为部署验收，不把密钥写入代码或文档。

家庭智能体及上述未完成能力的完整拆分和验收标准见 [M7-A4 家庭智能体与后续能力实施方案](m7-family-agent-roadmap.md)。不提前增加健康或儿童敏感数据；财务后续能力继续遵守 M8 的共享边界与确认流程。
...
- Hermes 生产模型提供方、模型别名、费用上限和故障回退策略。
- Agent 对话正文保留期、专用数据卷备份边界和首个真实消息渠道。
- NAS 部署时 Hermes 模型与消息平台的出口允许列表。

...
M2 最后一批聚焦固定浏览器回归、会话续期与撤销边界、请求级日志和正式部署配置收口。
...
下一次代码迭代继续完成 M2，不同时启动观影或访客 UI：

1. 使用 Playwright 固化 390 像素移动视口和桌面 Web 的关键家庭流程。
2. 明确账号与家庭成员的长期边界，补充安全续期、登出撤销和角色变化后的会话策略。
3. 为 API 增加请求 ID、结构化错误日志和不含敏感信息的家庭上下文。
4. 整理正式家庭部署的密钥、CORS、HTTPS、健康检查和备份配置模板。
5. 完成 M2 验收清单并冻结数据库基础约定。

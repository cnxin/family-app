...
## 附录七：A7.5 多步骤家庭协调提案与验收加固（2026-08-10）

### 已完成

| 批次 | 提交 | 交付内容 |
| --- | --- | --- |
| A7.5 多步骤家庭协调提案 | `cb256d7` | `agent_proposal_groups`、`propose_plan`、组提案预览与确认界面、跨菜单/购物/任务等模块的组合执行、API 与响应式回归 |
| Web 鉴权会话回归修复 | `d2c5d17` | 修复第三个登录 setup 触发限流，以及 refresh token 污染公共 storage state；Web 套件恢复为 43 passed / 8 skipped / 0 failed |
| 组提案验收与超时完善 | `9e6e75f` | 真机输入补齐必要信息；组提案允许路径 (a)/(b) 两种有效工具选择；Hermes chat 超时由 180 秒提高到 240 秒；最终真机走路径 (a)，生成 5 个子项 |

### 组合确认采用全有或全无

A7.5 采用整组确认或整组拒绝，不支持逐项确认。现有 `confirm()` 是单行乐观并发模型；若允许逐项确认，跨行依赖校验会引入父组与子项状态组合、部分成功后的重试规则、失效与再编辑规则，以及跨模块失败补偿，实质上需要一套新的状态机，不能沿用单行 `expectedVersion` 语义安全表达。

这一选择的代价是：用户即使只想修改其中一步，也要拒绝整组并重新生成，不能保留其余已接受步骤。当前阶段优先保证行为可理解、执行可审计和失败无残留；若未来逐项编辑成为高频需求，应作为独立批次设计状态机，而不是在现有确认接口上叠加例外。

全有或全无由同一个数据库事务保证，不依赖应用层补偿。回归测试会构造一个必然执行失败的子项，并断言事务结束后整组及此前已执行的子项全部回滚，没有留下部分任务、菜单或购物数据。这条断言验证的是数据库原子性，而不是失败后尽力删除的补偿逻辑。

### Web 套件回归与教训

登录失败由两个既有变化叠加触发：`dcaa85b` 增加第三个登录 setup，叠加 `5ba76d4` 的默认密码候选顺序后，请求次数超过登录限流；修复改为优先尝试当前开发账号实际使用的空密码，并复用同账号已成功的候选，避免重复失败登录。以后每新增一个登录 setup，都必须同时评估登录尝试总数与限流窗口，不能只验证 setup 自身。

另一处回归来自 A7.3（`772610a`）：刷新令牌恢复场景复用了公共 storage state，测试中的 refresh token 轮换污染后续用例，导致大量用例无法开始。修复为恢复场景使用独立 storage state，不再改写其他项目共享的认证状态。两处修复后，Web 套件为 43 passed / 8 skipped / 0 failed。

### 真机输入必须足以执行

初版输入“周六爸妈来吃饭”没有提供餐次、人数和忌口。模型先读取 7 类家庭数据再追问必要信息，是正确行为，与 A7.3 反向断言要求的信息不足时诚实降级一致，不是 `propose_plan` 的实现缺陷。

验收输入补齐为周六晚餐、总人数、忌口和明确的菜单/购物/准备任务目标后，模型走路径 (a)：调用 `propose_plan` 并生成 5 个子项。真机断言的输入必须自带足够执行信息，否则测到的是模型的追问倾向，而不是工具是否可见、可调用和能否正确落地。路径 (b)（分别调用菜单、购物和任务提案）仍保留为可接受的模型选择，但本次没有走该路径。

### 超时、授权与最终余量

Hermes chat 超时经历了三次取值：原始 90 秒时多工具场景撞顶并回落；A7.4-A 提高到 180 秒后工具与记忆场景可完成；A7.5 的 5 子项规划耗时更长，最终提高到 240 秒。

最终取值为：

```text
AGENT_HERMES_CHAT_TIMEOUT_MS = 240_000
AGENT_TOOL_AUTHORIZATION_TTL_MS = 360_000
授权余量 = 120_000
```

授权 TTL 从 300 秒提高到 360 秒，恢复 120 秒余量；它只延长工具授权的有效期，不延长 240 秒的 HTTP 请求上限。移动端没有更短的主动请求超时，仓库内也没有会被 360 秒授权 TTL 触发的反向代理业务请求上限；NAS 外部代理配置不在本仓库内，但本次没有增加连接持续时间。运行时契约不再硬编码某一个 chat 超时值，而是要求 chat 超时位于 120–300 秒之间，并始终保留至少 60 秒工具授权余量，锁定真正需要维护的取值关系。

调整后完整真机回归再次通过：工具 9/9、记忆 2/2、页面上下文 2/2、组提案 1/1，全部由 Hermes 完成且回落数为 0；组提案继续走路径 (a)，生成 5 个子项，耗时 153.9 秒。

### 遗留待办

A7.3 的单实体指代约束目前只覆盖 `search_recipes`。其余 4 类 `entityType`（`asset` / `knowledge` / `travel` / `poll`）要在 A6.1 铺开页面上下文入口时，为对应工具补同类约束与反向断言，避免用户离开详情页后用“这个”提问时模型自行选择家庭资源。

---

## 附录八：A6.1 信息架构收敛落地记录（2026-08-10）

### 已完成

| 任务 | 提交 | 交付内容 |
| --- | --- | --- |
| 页面上下文入口铺开 | `849069c` | 家庭资产、家庭知识库、家庭出行、家庭投票四页新增全宽「问小管家」入口；API 补齐四类实体解析与家庭隔离回归；Mobile 补齐四视口和触控回归 |
| 单实体指代约束 | `6fd7ced` | `search_knowledge` 增加知识文章单实体约束；真机增加无页面上下文时必须追问具体文章的反向断言 |
| 业务页顶部入口验收 | `705697b` | 固化四个业务页 390 x 844 真实页面截图，确认入口位于页头之后、主体内容之前，且保留底部小管家 tab |
| 附录八与最终验收 | 本附录所在提交 | 记录 A6.1 取舍、跳过项、最终回归结果和恢复入口 |

### 路由枚举与上下文粒度

任务开始前枚举了 `apps/mobile/src/app/` 的实际路由，没有假设动态详情页存在：

| 实体类型 | 实际路由形态 | 接入粒度 | 处理结果 |
| --- | --- | --- | --- |
| `asset` | 只有 `/home-assets` 对应的 `(tabs)/assets.tsx`；详情通过同页弹层处理 | 仅 `route=/home-assets` | 已接入 |
| `knowledge` | 只有 `/knowledge` 对应的 `(tabs)/knowledge.tsx`；详情通过同页状态处理 | 仅 `route=/knowledge` | 已接入 |
| `travel` | `/travel?planId=...` 复用 `(tabs)/travel.tsx`，没有独立动态详情路由 | 顶部入口仅传 `route=/travel` | 已接入 |
| `poll` | 只有 `/polls` 对应的 `(tabs)/polls.tsx`；详情与编辑通过同页处理 | 仅 `route=/polls` | 已接入 |

四个入口均没有伪造 `entityType` 或 `entityId`。服务端仍只信任 JWT，并对四类实体正常 ID、跨家庭 ID、不存在 ID 和未知类型做了回归；跨家庭 ID 与不存在 ID 生成一致的无上下文结果，避免资源存在性探测。

### 单实体指代约束的最终范围

`search_knowledge` 的最终 description 为：

> 搜索家庭知识库的标题与摘要。用户以“这篇文章”“这条知识”等词指代单个知识条目、但既无具体标题也无页面上下文时，不得猜测条目或调用无条件搜索，应先追问具体文章；“搜索关于报销的资料”等范围查询应直接调用本工具。

真机输入“这篇文章主要讲了什么”时，Hermes 不调用 `search_knowledge`，明确要求文章内容、标题或链接。约束前后 A7.4-A 的 9 个范围查询均为 9/9，记忆 2/2、组提案 1/1，回落数为 0。

其余三类没有伪造完成状态：

- `asset` 没有只读 Agent 工具，无法在工具 description 层承载约束；本批未擅自新增 `get_asset_maintenance`。
- `poll` 只有写入型 `propose_poll`，没有只读查询工具；写入提案 description 不能约束投票查询，本批未擅自新增 `get_poll_status`。
- `travel` 有 `get_travel_checklist`，但工具参数允许缺省 `travelPlanId` 并由 service 选择首个活动行程。初版约束和一次加固约束都保持 A7.4-A 9/9；专项运行曾正确追问，但两次完整真机运行仍先调用日程/清单工具、选择首项回答，再补一句“不确定你指哪一个”。按“不反复试探”的刹车规则，最终回退 description 与反向断言。要稳定解决，需先取消 service 的首项缺省语义，或为按名称/日期解析行程设计显式契约，不能只依赖模型遵守文字描述。

### 顶部入口与底部 tab 的决策

保留底部小管家 tab。底部入口承载无页面上下文的自由咨询，是现有用户习惯；业务页顶部入口的价值是让用户在当前模块就近发问，不是替代独立会话入口。移除 tab 会形成破坏性导航变更，且无法改善上下文真实性。

顶部入口只放在本批实际接入的家庭资产、知识库、出行和投票页面。任务 1 已把按钮放在各页标题/主操作之后、筛选和主体内容之前，并沿用菜品页的 `Sparkles + 文本 + ChevronRight` 全宽样式；因此任务 3 没有重复移动组件或再造第二套入口，而是补齐四张 390px 真实截图和全量回归证据。四页都能容纳入口，没有页面因 header 空间不足而跳过；没有修改 `app-shell.tsx`、全局 system prompt 或底部 tab。

四视口 320/375/390/414px 均验证入口不少于 44px、文字容器使用 `flex: 1` 与 `minWidth: 0`、页面无横向溢出。Web 全量保持 43 passed / 8 skipped / 0 failed，51 项全部得到明确结果，没有 `not run`。

### 遗留待办与恢复入口

- A7.3 的页面上下文白名单仍缺 `task` / `menu` / `media` / `visit`。四类都缺可供 `AgentPageContextService` 调用的按 ID 单查 service 方法；`menu` 的 `get()` 是带 `@CurrentUser` / `@Query` 装饰器的控制器方法，不能内部调用。后续必须先新增家庭隔离的 service 查询契约，再扩展白名单和反向断言。
- `asset` / `poll` 的单实体约束需要先决定是否新增只读工具，并同步 MCP schema、两份 Hermes 白名单、授权默认值、卡片和端到端回归。
- `travel` 的单实体约束应从工具执行契约入手：无 `travelPlanId` 时不得默认选择首项，或先提供结构化的行程解析工具。恢复时从 service 行为和 API 回归开始，不要继续只改 description。
- 本批没有新增表、列、索引或迁移；`synchronize` 保持 `false`。

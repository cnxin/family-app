...
面向家庭成员的生活协作平台。当前已完成点菜、菜单、菜谱、购物清单、家庭库存和统一日历的本地 Web 演示版，后续将扩展提醒、任务与投票、观影、访客和家庭资产等模块。

> 当前可用流程：家人提前安排菜单和家庭事件 -> 指定本餐主厨或分别认领菜品 -> 生成购物清单 -> 上桌后锁定历史菜单 -> 管理家庭库存与补货
...
# 家庭隔离、统一日历、多人菜谱、菜单做法快照、采购、菜单协作、通知、审计、健康检查、请求日志、敏感信息保护、
# 唯一约束和事务回滚，结束后自动清理
...
# 运行前保持 API/数据库健康；日历事件在用例内创建、编辑并删除，不保留测试数据
